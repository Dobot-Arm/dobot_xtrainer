...
import os
import json
from dobot_function.function_utils import log_write, time_print
...
class DobotApi:
    ...
    def wait_reply(self):
        ...
        try:
            ...
            # log_write(__file__, f'Receive from {self.ip}:{self.port}: {data_str}')
            return data_str
...
class DobotApiStatus(DobotApi):
    def get_error(self):
        ...
        if hex((feedInfo['test_value'][0])) == '0x123456789abcdef':
            rt_bool = feedInfo['error_status'][0]
            # time_print(rt_bool)
        else:
            rt_bool = True
        return rt_bool
