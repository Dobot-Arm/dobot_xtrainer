...
from dobot_function.function_utils import wait_period, log_write, mk_dir, scan_port
import numpy as np
...
class DynamixelDriver(DynamixelDriverProtocol):
    def __init__(
        self, ids: Sequence[int], append_id: int, port: str = "/dev/ttyUSB0", baudrate: int = 1000000, using_sensor = True
    ):
    ...
    def set_pid_P(self):
        ...
        id_all.sort()
        para = [1000, 2000, 2000, 600, 1500, 1500, 1500]
        # para = [460, 460, 460, 400, 400, 400, 400]
        for i in range(len(id_all)):
            self.set_joint_arg_2Byte(id_all[i], 84, para[i])

    def set_PwmLimit(self):
        id_all = list(tuple(self._ids)[:6] + (self._append_id,))
        id_all.sort()
        para = [250, 250, 250, 250, 250, 250, 250]
        self.set_torque_mode(False)
        # para = [460, 460, 460, 400, 400, 400, 400]
        for i in range(len(id_all)):
            self.set_joint_arg_2Byte(id_all[i], 36, para[i])
...
def main():
    ...
    driver1 = DynamixelDriver(ids_left, 3, "/dev/ttyUSB1")
